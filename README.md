...
**สิทธิ์ผู้ใช้ (owner / staff):** role เก็บใน **app_metadata** ของ user
(ไม่ใช่ user_metadata — user แก้เองไม่ได้). dashboard เวอร์ชันใหม่ไม่มีปุ่มแก้
metadata จากหน้า Users แล้ว — ตั้งค่าผ่าน **SQL Editor → New query**:

```sql
-- ทำให้ user เป็น staff
update auth.users
...
- ไม่ใส่ / ไม่ใช่ `"staff"` → ถือเป็น **owner** (เข้าได้ทุกหน้า)
- `"role": "staff"` → เข้าได้แค่ **จดออเดอร์ + ประวัติ**; `/dashboard`, `/menu`
  ถูกบล็อก (redirect กลับ `/order`) ทั้งที่ `proxy.ts` และ server ของแต่ละหน้า
- หลังเปลี่ยน role ต้อง **logout/login ใหม่** ค่าถึงจะมีผล (JWT refresh)
